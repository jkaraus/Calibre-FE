...
const AuthorRow = React.memo<{
  ...
  <TableRow onClick={() => onSelect(author)} sx={{ cursor: 'pointer', '&:hover': { backgroundColor: 'action.hover' } }}>
...
const Authors: React.FC = () => {
  ...
  const authorsSortFn = useCallback((a: Author, b: Author) => {
    ...
    <Container maxWidth={false} sx={{ py: 4, px: 4 }}>
      {!selectedAuthor && !selectedSeries ? (
        ...
        <>
          ...
          {filteredAuthors && filteredAuthors.length > 0 ? (
            ...
            <Box textAlign="center" py={4}>
              <Typography variant="body1" color="text.secondary">
                {authorsSearchTerm.trim() ? 'Žádní autoři odpovídající vašemu vyhledávání nebyli nalezeni.' : 'Žádní autoři nebyly nalezeni.'}
              </Typography>
            </Box>
          )}
        </>
        ...
        <DetailView
          ...
          description={formatBookDescription("autora", selectedAuthor.bookCount, "knihy od tohoto")}
