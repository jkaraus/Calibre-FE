...
- **Domů** - Nejnovější knihy a přehled knihovny
...
- **[React 19.2](https://react.dev/)** - Nejnovější React s improved performance
- **[TypeScript 5.9](https://www.typescriptlang.org/)** - Type safety a IntelliSense
- **[Vite 7.2](https://vitejs.dev/)** - Lightning-fast build tool a HMR
- **[Material-UI v7.3](https://mui.com/)** - Modern React component library

### State Management & Data Fetching

- **[TanStack React Query 5.90](https://tanstack.com/query)** - Server state, caching, background updates
- **[TanStack Router 1.157](https://tanstack.com/router)** - Type-safe routing solution
- **[Zustand 5.0](https://github.com/pmndrs/zustand)** - Lightweight client state management
...
│   ├── DetailHeader.tsx   # Hlavníčka pro detail view s back buttonem
│   ├── DetailView.tsx     # Kompletní detail view komponenta
│   ├── LoadMoreButton.tsx # Standardizované tlačítko pro načítat více
│   ├── SearchInput.tsx    # Konzistentní vyhledávací input komponenta
│   └── SkeletonLoader.tsx # Univerzální skeleton loading komponenty
...
├── hooks/                # Custom hooks pro sdílenou funkcionalitu
│   ├── useApi.ts         # API integration hooks
│   ├── useDebounced.ts   # Debounced values s 150ms default delay
│   ├── useListSearch.ts  # Kombinovaný search, filtering a progressive loading
│   ├── useProgressiveLoading.ts # Progressive loading s infinite scroll
│   └── useScrollPosition.ts # Automatické ukládání scroll pozice
├── services/             # API layer s React Query integracíí
│   ├── api.ts           # Centralizované API hooks a error handling
│   └── queryClient.ts   # TanStack Query client konfigurace
...
└── utils/            # Utility functions
    ├── constants.ts  # App konstanty a query keys
    └── localization.ts # České lokalizační funkce
```
